...
const STAFF = [
  ...
  { id: 's2', name: 'Rabbi Klein', role: 'Teacher' },
  { id: 's3', name: 'Rabbi Goldstein', role: 'Teacher' },
  { id: 's4', name: 'Mrs. Goldberg', role: 'Speech Therapist' },
  { id: 's5', name: 'Mr. Weinstein', role: 'OT' },
  { id: 's6', name: 'Mrs. Friedman', role: 'Counselor' },
  { id: 's7', name: 'Ezriel', role: 'BT' },
  { id: 's8', name: 'Dovid', role: 'BT' },
]
const DAYS = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri']

const initialStudents = [
  { id: 1, name: 'Bloom Yair', points: 45, redMarks: 2, att: ['P','P','L','L','L','P'], breakfast: ['Y','Y','N','N','N','Y'], detention: true, status: 'present', withStaff: null, services: [{ staffId: 's4', type: 'Speech Therapy', hrs: 1.5 }], parentCalls: [{ date: '2025-05-28', staff: 'Rabbi Klein', notes: 'Discussed attendance issues', duration: '8 min' }], notes: [{ date: '2025-05-30', author: 'Rabbi Klein', text: 'Improving in morning davening.' }], behaviorLog: [], iep: true, iepDetails: 'Speech IEP - annual review due Aug 2025' },
  { id: 2, name: 'Friedlander Zev', points: 80, redMarks: 0, att: ['A','A','P','P','P','P'], breakfast: ['Y','Y','Y','Y','Y','Y'], detention: false, status: 'present', withStaff: null, services: [], parentCalls: [], notes: [], behaviorLog: [], iep: false, iepDetails: '' },
  { id: 3, name: 'Haddad Moshe Chaim', points: 60, redMarks: 3, att: ['P','L','A','P','P','P'], breakfast: ['N','N','N','Y','Y','Y'], detention: false, status: 'therapy', withStaff: 's6', services: [{ staffId: 's6', type: 'Counseling', hrs: 3 }], parentCalls: [{ date: '2025-06-01', staff: 'Mrs. Cohen', notes: 'Left voicemail', duration: '2 min' }], notes: [], behaviorLog: [], iep: false, iepDetails: '' },
  { id: 4, name: 'Hayon David', points: 95, redMarks: 0, att: ['P','P','P','P','P','P'], breakfast: ['Y','Y','Y','Y','Y','Y'], detention: false, status: 'present', withStaff: null, services: [], parentCalls: [], notes: [{ date: '2025-06-02', author: 'Rabbi Klein', text: 'Excellent week overall.' }], behaviorLog: [], iep: false, iepDetails: '' },
  { id: 5, name: 'Karman Yitzchok', points: 20, redMarks: 5, att: ['A','A','A','P','P','P'], breakfast: ['N','N','N','N','Y','Y'], detention: false, status: 'absent', withStaff: null, services: [], parentCalls: [], notes: [], behaviorLog: [], iep: false, iepDetails: '' },
  { id: 6, name: 'Levitz Avrohom', points: 70, redMarks: 1, att: ['P','P','P','L','P','P'], breakfast: ['Y','N','N','N','Y','Y'], detention: false, status: 'hallway', withStaff: 's7', services: [{ staffId: 's5', type: 'OT', hrs: 2 }], parentCalls: [{ date: '2025-05-20', staff: 'Rabbi Klein', notes: 'General check-in', duration: '5 min' }], notes: [], behaviorLog: [], iep: true, iepDetails: 'OT IEP - sensory processing' },
  { id: 7, name: 'Rosenfeld Yehuda', points: 55, redMarks: 6, att: ['P','P','P','P','A','P'], breakfast: ['Y','Y','Y','Y','Y','Y'], detention: true, status: 'late', withStaff: null, services: [], parentCalls: [], notes: [], behaviorLog: [], iep: false, iepDetails: '' },
  { id: 8, name: 'Schwartz Moishe Michael', points: 40, redMarks: 2, att: ['L','L','L','L','P','P'], breakfast: ['Y','Y','Y','N','N','Y'], detention: false, status: 'present', withStaff: null, services: [{ staffId: 's6', type: 'Counseling', hrs: 0.5 }], parentCalls: [], notes: [], behaviorLog: [], iep: false, iepDetails: '' },
  { id: 9, name: 'Simon Eliyahu', points: 65, redMarks: 1, att: ['P','P','P','P','P','P'], breakfast: ['Y','Y','Y','Y','N','Y'], detention: false, status: 'not-arrived', withStaff: 's7', services: [], parentCalls: [], notes: [], behaviorLog: [], iep: false, iepDetails: '' },
]
...
const AVATAR_COLORS = ['#1e3a5f','#374151','#1e4d2b','#713f12','#4c1d95','#164e63','#831843','#14532d','#7c2d12']
...
const S = {
  ...
  sidebarItem: (active) => ({ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 16px', cursor: 'pointer', borderRadius: 6, margin: '1px 8px', background: active ? 'rgba(255,255,255,0.12)' : 'transparent', color: active ? '#fff' : 'rgba(255,255,255,0.6)', fontSize: 13.5, fontWeight: active ? 600 : 400, transition: 'all 0.15s' }),
...
function LoginPage({ onLogin }) {
  ...
  const accounts = {
    ...
    therapist: { name: 'Mrs. Goldberg', email: 'mgoldberg@hadranacademy.org' },
  }
  return (
    <div style={{ minHeight: '100vh', background: '#1a1f36', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'DM Sans',sans-serif" }}>
      <div style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', width: 820, display: 'flex', boxShadow: '0 24px 80px rgba(0,0,0,0.4)' }}>
        <div style={{ flex: 1, padding: '52px 40px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#1a1f36' }}>
          ...
          <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, marginTop: 10, textAlign: 'center', lineHeight: 1.6 }}>Student Management System</div>
          <div style={{ marginTop: 40, display: 'flex', flexDirection: 'column', gap: 12, width: '100%' }}>
            {[['🎓', 'Menahel Dashboard'], ['👨‍🏫', 'Teacher Portal'], ['🧠', 'Therapist Access'], ['🛒', 'Canteen Store']].map(([icon, label]) => (
              <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'rgba(255,255,255,0.5)', fontSize: 13 }}>
        ...
        <div style={{ background: '#1a1f36', padding: '18px 24px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
              ...
              {withStaffObj && <span style={{ background: 'rgba(255,255,255,0.15)', color: '#fff', padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600 }}>👤 With {withStaffObj.name} ({withStaffObj.role})</span>}
          ...
          <div style={{ display: 'flex', gap: 20, color: '#fff', textAlign: 'center' }}>
            ...
            <div><div style={{ fontSize: 20, fontWeight: 800, color: s.redMarks >= 6 ? '#f87171' : '#fff' }}>{s.redMarks}</div><div style={{ fontSize: 10, opacity: 0.6 }}>Red Marks</div></div>
          ...
          {tab === 'overview' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div style={S.card}>
                ...
                {[['Present days', s.att.filter(d=>d==='P').length+'/6'],['Late arrivals', lateCount],['Absences', absCount],['Points balance', s.points+' pts'],['Red marks', s.redMarks],['Last parent call', lastCall ? daysSince(lastCall.date)+'d ago' : 'Never']].map(([label, val]) => (
              ...
              <div style={S.card}>
                <div style={{ fontWeight: 700, marginBottom: 12, fontSize: 14 }}>Attendance This Week</div>
                <div style={{ display: 'flex', gap: 6 }}>
                  {DAYS.map((day, i) => (
                    <div key={day} style={{ flex: 1, textAlign: 'center' }}>
                      <div style={{ fontSize: 10, color: '#6b7280', marginBottom: 4 }}>{day}</div>
                      <div style={{ width: 32, height: 32, borderRadius: 6, background: s.att[i]==='P'?'#dcfce7':s.att[i]==='A'?'#fee2e2':'#dbeafe', color: s.att[i]==='P'?'#16a34a':s.att[i]==='A'?'#dc2626':'#2563eb', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 12, margin: '0 auto' }}>{s.att[i]}</div>
                    </div>
                  ))}
                </div>
              </div>
              {withStaffObj && (
                <div style={{ ...S.card, gridColumn: 'span 2', borderLeft: '3px solid #0891b2' }}>
                  <div style={{ fontWeight: 700, color: '#0891b2', marginBottom: 4, fontSize: 13 }}>📍 Currently With</div>
                  <div style={{ fontSize: 14 }}><strong>{withStaffObj.name}</strong> — {withStaffObj.role}</div>
                </div>
              )}
                ...
                <tbody>
                  {['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday'].map((day, i) => (
                    <tr key={day} style={{ borderBottom: '1px solid #f4f5f7' }}>
                      ...
                      <td style={{ padding: 10, textAlign: 'center' }}><span style={S.badge(s.breakfast[i]==='Y'?'#166534':'#dc2626', s.breakfast[i]==='Y'?'#dcfce7':'#fee2e2')}>{s.breakfast[i]==='Y'?'✓ Had breakfast':'✗ Skipped'}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {tab === 'behavior' && (
            <div style={S.card}>
              <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
                ...
                <div style={{ ...S.statCard('#dc2626'), flex: 1 }}><div style={{ fontSize: 11, color: '#6b7280' }}>Red Marks</div><div style={{ fontSize: 26, fontWeight: 800, color: '#dc2626' }}>{s.redMarks}</div></div>
              </div>
              <div style={{ fontWeight: 700, marginBottom: 10, fontSize: 14 }}>Behavior Log</div>
...
export default function Dashboard() {
  ...
  function updateWithStaff(id, staffId) { setStudents(prev => prev.map(s => s.id === id ? { ...s, withStaff: staffId || null } : s)) }
  ...
  function addRedMark(id) {
    ...
    playSound(s && s.redMarks + 1 >= 6 ? 'redmark' : 'negative')
    setStudents(prev => prev.map(s => s.id === id ? { ...s, redMarks: s.redMarks + 1 } : s))
  }
        ...
        {page === 'dashboard' && (
          <div>
            <div style={{ marginBottom: 24 }}>
              ...
              <p style={{ color: '#6b7280', margin: '4px 0 0', fontSize: 13 }}>Dargei Beis · Week of Jun 2, 2025 · {students.length} students</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12, marginBottom: 24 }}>
              {[['Present', present, '#2563eb'],['Absent', absent, '#dc2626'],['Late', late, '#d97706'],['In Therapy', inTherapy, '#7c3aed'],['In Hallway', inHallway, '#0891b2'],['Not Arrived', notArrived, '#6b7280']].map(([label, val, color]) => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div style={{ ...S.card, gridColumn: 'span 2' }}>
                <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 14, color: '#1a1f36' }}>Student Status — Live</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                  {students.map((s, i) => {
                    ...
                    const withStaffObj = s.withStaff ? STAFF.find(st => st.id === s.withStaff) : null
                    return (
                      <div key={s.id} onClick={() => setSelectedStudent(s)} style={{ background: '#fafafa', border: '1px solid #e8eaed', borderRadius: 8, padding: '12px 14px', cursor: 'pointer', transition: 'border-color 0.15s' }}>
                        ...
                        {withStaffObj && <div style={{ fontSize: 11, color: '#0891b2', fontWeight: 600, marginBottom: 6 }}>👤 With {withStaffObj.name}</div>}
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                          ...
                          {needsCall && <span style={S.badge('#92400e', '#fff7ed')}>📞?</span>}
                          {s.iep && <span style={S.badge('#5b21b6', '#f5f3ff')}>IEP</span>}
                        </div>
                      </div>
                    )
                  })}
              ...
              <div style={S.card}>
                <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 12, color: '#1a1f36' }}>Top Alerts</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 260, overflow: 'auto' }}>
                  {alerts.slice(0, 8).map((a, i) => (
                    <div key={i} onClick={() => { const s = students.find(x => x.id === a.id); if (s) setSelectedStudent(s) }} style={{ background: a.type === 'danger' ? '#fef2f2' : a.type === 'warn' ? '#fffbeb' : '#eff6ff', border: `1px solid ${a.type === 'danger' ? '#fecaca' : a.type === 'warn' ? '#fde68a' : '#bfdbfe'}`, borderRadius: 6, padding: '8px 12px', cursor: 'pointer' }}>
                      <div style={{ fontWeight: 600, fontSize: 12 }}>{a.student}</div>
                      <div style={{ fontSize: 11, color: '#555', marginTop: 1 }}>{a.msg}</div>
                    </div>
                  ))}
                  {alerts.length === 0 && <div style={{ color: '#9ca3af', textAlign: 'center', padding: '1rem', fontSize: 13 }}>All clear ✅</div>}
                </div>
              </div>
              <div style={S.card}>
                <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 12, color: '#1a1f36' }}>Parent Calls Needed</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 260, overflow: 'auto' }}>
                  {students.map((s, i) => {
                    const lastCall = s.parentCalls.length > 0 ? s.parentCalls[s.parentCalls.length - 1] : null
                    ...
                    return (
                      <div key={s.id} onClick={() => setSelectedStudent(s)} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 6, padding: '8px 12px', cursor: 'pointer' }}>
                        <div style={S.avatar(i, 28)}>{initials(s.name)}</div>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 600, fontSize: 12 }}>{s.name}</div>
                          <div style={{ fontSize: 11, color: '#92400e' }}>{lastCall ? `Last called ${days} days ago` : 'Never called'}</div>
                        </div>
                      </div>
                    )
                  }).filter(Boolean)}
                  ...
                  <div key={s.id} onClick={() => setSelectedStudent(s)} style={{ ...S.card, display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', padding: '14px 18px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, marginTop: 4, flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        {withStaffObj && <span style={{ fontSize: 11, color: '#0891b2', fontWeight: 600 }}>👤 With {withStaffObj.name}</span>}
                    ...
                    <div style={{ display: 'flex', gap: 20, textAlign: 'center' }}>
                      ...
                      <div><div style={{ fontSize: 17, fontWeight: 800, color: s.redMarks >= 4 ? '#dc2626' : '#374151' }}>{s.redMarks}</div><div style={{ fontSize: 10, color: '#9ca3af' }}>red marks</div></div>
                      ...
                      <div><div style={{ fontSize: 13, fontWeight: 600 }}>{lastCall ? `${daysSince(lastCall.date)}d ago` : 'Never'}</div><div style={{ fontSize: 10, color: '#9ca3af' }}>last call</div></div>
                    </div>
                  </div>
                )
              })}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #e8eaed' }}>
                    ...
                    <th style={{ padding: 8, textAlign: 'center' }}>With</th>
                  ...
                  {filteredStudents.map((s, i) => {
                    ...
                    return (
                      <tr key={s.id} style={{ borderBottom: '1px solid #f4f5f7' }}>
                        ...
                        <td style={{ padding: 8, textAlign: 'center', fontSize: 11, color: '#0891b2', fontWeight: 600 }}>{withStaffObj ? withStaffObj.name : '—'}</td>
                        {s.att.map((d, j) => (
                        ...
                        {role !== 'therapist' && (
                          <td style={{ padding: '4px 8px' }}>
                            <div style={{ display: 'flex', gap: 4, flexDirection: 'column' }}>
                              <select value={s.status} onChange={e => updateStatus(s.id, e.target.value)} style={{ padding: '3px 6px', borderRadius: 4, border: '1px solid #e5e7eb', fontSize: 11 }}>
                                {Object.entries(statusLabel).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                              </select>
                              <select value={s.withStaff || ''} onChange={e => updateWithStaff(s.id, e.target.value)} style={{ padding: '3px 6px', borderRadius: 4, border: '1px solid #e5e7eb', fontSize: 11 }}>
                                <option value=''>— Not with staff —</option>
            ...
            {behaviorStudent ? (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: '#d97706', fontWeight: 700, fontSize: 13 }}>{students.find(s => s.id === behaviorStudent.id)?.points} pts · {students.find(s => s.id === behaviorStudent.id)?.redMarks} red marks</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                  <button onClick={() => setBehaviorTab('positive')} style={S.btn(behaviorTab === 'positive' ? 'success' : 'ghost')}>✅ Positive Behaviors</button>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                {[...students].sort((a, b) => b.points - a.points).map((s, i) => (
                  <div key={s.id} onClick={() => setBehaviorStudent(s)} style={{ ...S.card, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px' }}>
                    <div style={S.avatar(s.id - 1, 38)}>{initials(s.name)}</div>
                    <div style={{ flex: 1 }}>
        ...
        {page === 'store' && (
          <div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {students.map(s => (
                  <button key={s.id} onClick={() => setStoreStudent(s.id)} style={{ padding: '6px 12px', borderRadius: 6, border: `2px solid ${storeStudent === s.id ? '#1a1f36' : '#e5e7eb'}`, cursor: 'pointer', fontSize: 12, fontWeight: storeStudent === s.id ? 700 : 400, background: storeStudent === s.id ? '#1a1f36' : '#fff', color: storeStudent === s.id ? '#fff' : '#374151' }}>
                    {s.name} · {s.points} pts
                  </button>
                ))}
              </div>
            ...
            {storeStudent && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                {STORE_ITEMS.map(item => {
                  ...
                  return (
                    <div key={item.id} style={{ ...S.card, textAlign: 'center', opacity: canAfford ? 1 : 0.5 }}>
                      <div style={{ fontSize: 36, marginBottom: 8 }}>{item.emoji}</div>
                      <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>{item.name}</div>
                      <div style={{ color: '#d97706', fontWeight: 800, fontSize: 16, marginBottom: 12 }}>{item.cost} pts</div>
        ...
        {page === 'alerts' && (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {alerts.map((a, i) => (
                <div key={i} onClick={() => { const s = students.find(x => x.id === a.id); if (s) setSelectedStudent(s) }} style={{ background: a.type === 'danger' ? '#fef2f2' : a.type === 'warn' ? '#fffbeb' : '#eff6ff', border: `1px solid ${a.type === 'danger' ? '#fecaca' : a.type === 'warn' ? '#fde68a' : '#bfdbfe'}`, borderRadius: 8, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}>
                  <div><div style={{ fontWeight: 700, fontSize: 13 }}>{a.student}</div><div style={{ fontSize: 12, color: '#555', marginTop: 2 }}>{a.msg}</div></div>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {students.map((s, i) => {
                ...
                const days = lastCall ? daysSince(lastCall.date) : 999
                return (
                  <div key={s.id} onClick={() => setSelectedStudent(s)} style={{ ...S.card, cursor: 'pointer', borderLeft: `3px solid ${days > 14 ? '#f97316' : '#16a34a'}`, padding: '14px 18px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                      <div style={S.avatar(i, 32)}>{initials(s.name)}</div>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 13 }}>{s.name}</div>
                        <div style={{ fontSize: 11, color: days > 14 ? '#ea580c' : '#16a34a', fontWeight: 600 }}>{lastCall ? `Last call: ${days} days ago` : '⚠️ Never called'}</div>
                      </div>
                      <div style={{ marginLeft: 'auto', fontSize: 11, color: '#9ca3af' }}>{s.parentCalls.length} calls</div>
                    </div>
                    {lastCall && <div style={{ fontSize: 12, color: '#6b7280', background: '#f4f5f7', borderRadius: 6, padding: '6px 10px' }}>{lastCall.notes}</div>}
                  </div>
                )
              })}
            </div>

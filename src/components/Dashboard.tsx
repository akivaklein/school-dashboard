...
function getImprovement(s) {
  if (s.lastWeekReminders === 0 && s.reminders === 0) return { label: '✅ No reminders', color: '#16a34a' }
  if (s.reminders < s.lastWeekReminders) return { label: `📈 Improved! (${s.lastWeekReminders} → ${s.reminders})`, color: '#16a34a' }
  if (s.reminders > s.lastWeekReminders) return { label: `📉 More reminders (${s.lastWeekReminders} → ${s.reminders})`, color: '#dc2626' }
  return { label: `➡️ Same as last week (${s.reminders})`, color: '#d97706' }
}
          ...
          {tab === 'overview' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div style={S.card}>
                  <div style={{ fontWeight: 700, marginBottom: 10, fontSize: 14 }}>📈 Improvement</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: improvement.color }}>{improvement.label}</div>
                </div>
                <div style={S.card}>
                  <div style={{ fontWeight: 700, marginBottom: 10, fontSize: 14 }}>Attendance</div>
                  <div style={{ display: 'flex', gap: 6 }}>
                    {DAYS.map((day, i) => (
          ...
          {tab === 'behavior' && (
            <div>
              <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
                ...
                <div style={{ ...S.statCard(improvement.color), flex: 1 }}><div style={{ fontSize: 11, color: '#6b7280' }}>vs Last Week</div><div style={{ fontSize: 14, fontWeight: 700, color: improvement.color, marginTop: 4 }}>{improvement.label}</div></div>
              </div>
              <div style={S.card}>
...
// ─── TEACHING MODE ───────────────────────────────────────────────────────────
function TeachingMode({ students, setStudents, onExit }) {
  const [search, setSearch] = useState('')
  const [selected, setSelected] = useState([])
  const [activeBeh, setActiveBeh] = useState(null)

  function toggleSelect(id) {
    setSelected(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])
  }
  function selectAll() { setSelected(students.map(s => s.id)) }
  ...
  function applyToSelected(amount, label) {
    ...
    setStudents(prev => prev.map(s => selected.includes(s.id) ? {
      ...
      behaviorLog: [{ label, points: amount, date: new Date().toISOString().slice(0,10) }, ...s.behaviorLog].slice(0, 20)
    } : s))
    setSelected([])
    setActiveBeh(null)
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#f4f5f7', zIndex: 200, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: '#1a1f36', padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 16 }}>
        <div style={{ fontSize: 16, fontWeight: 800, color: '#fff' }}>🏫 Teaching Mode</div>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search students..." style={{ padding: '7px 12px', borderRadius: 6, border: 'none', fontSize: 13, width: 200, background: 'rgba(255,255,255,0.15)', color: '#fff' }} />
        <div style={{ display: 'flex', gap: 8, marginLeft: 'auto' }}>
          <button onClick={selectAll} style={S.btn('ghost')}>☑ Select All</button>
          <button onClick={clearAll} style={S.btn('ghost')}>✕ Clear</button>
          <button onClick={onExit} style={S.btn('danger')}>Exit Teaching Mode</button>
        </div>
      </div>

      {/* Behavior panel when students selected */}
      {selected.length > 0 && (
        <div style={{ background: '#fff', borderBottom: '1px solid #e8eaed', padding: '12px 24px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#1a1f36' }}>{selected.length} student{selected.length > 1 ? 's' : ''} selected:</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {BEHAVIORS_POSITIVE.map(b => (
              <button key={b.id} onClick={() => applyToSelected(b.points, b.label)} style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid #86efac', background: '#f0fdf4', color: '#166534', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>+{b.points} {b.label}</button>
            ))}
            {BEHAVIORS_NEGATIVE.map(b => (
              <button key={b.id} onClick={() => applyToSelected(b.points, b.label)} style={{ padding: '5px 10px', borderRadius: 6, border: '1px solid #fca5a5', background: '#fef2f2', color: '#dc2626', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>{b.points} {b.label}</button>
            ))}
            <button onClick={() => applyToSelected(10, 'Bonus points')} style={{ ...S.btn('success'), padding: '5px 12px', fontSize: 12 }}>+10 pts</button>
            <button onClick={() => applyToSelected(-10, 'Points deducted')} style={{ ...S.btn('danger'), padding: '5px 12px', fontSize: 12 }}>-10 pts</button>
          </div>
        </div>
      )}
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 24 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}>
          {filtered.map((s, i) => {
            ...
            return (
              <div key={s.id} style={{ background: '#fff', border: `2px solid ${isSelected ? '#1a1f36' : '#e8eaed'}`, borderRadius: 10, padding: '14px', cursor: 'pointer', position: 'relative' }}
                onClick={() => toggleSelect(s.id)}>
                {isSelected && <div style={{ position: 'absolute', top: 8, right: 8, width: 20, height: 20, borderRadius: '50%', background: '#1a1f36', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700 }}>✓</div>}
                ...
                <div style={{ display: 'flex', gap: 4, marginTop: 10 }} onClick={e => e.stopPropagation()}>
                  <button onClick={() => { playSound('positive'); setStudents(prev => prev.map(x => x.id === s.id ? {...x, points: x.points + 2, behaviorLog: [{label:'Quick +2', points:2, date: new Date().toISOString().slice(0,10)}, ...x.behaviorLog]} : x)) }} style={{ flex: 1, padding: '4px', borderRadius: 5, border: '1px solid #86efac', background: '#f0fdf4', color: '#166534', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>+2</button>
...
export default function Dashboard() {
  ...
  function applyBehavior(studentId, beh) {
    ...
    setStudents(prev => prev.map(s => {
      ...
      return { ...s, points: Math.max(0, s.points + beh.points), reminders: beh.points < 0 ? s.reminders + 1 : s.reminders, behaviorLog: [{ label: beh.label, points: beh.points, date: new Date().toISOString().slice(0,10) }, ...s.behaviorLog].slice(0, 20) }
    }))
  }
  ...
  if (teachingMode) return <TeachingMode students={students} setStudents={setStudents} onExit={() => setTeachingMode(false)} />
  ...
  return (
    <div style={S.app}>
        ...
        <div style={{ flex: 1, paddingTop: 4 }}>
          ...
          {role !== 'therapist' && (
            <div onClick={() => setTeachingMode(true)} style={{ ...S.sidebarItem(false), background: 'rgba(255,255,255,0.08)', margin: '8px 8px 2px', border: '1px solid rgba(255,255,255,0.15)' }}>
              <span>🏫</span><span>Teaching Mode</span>
            </div>
          )}
        </div>
      ...
      <div style={S.main}>
        ...
        {/* Search bar */}
        <div style={{ marginBottom: 20 }}>
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 Search students by name..." style={{ padding: '9px 14px', borderRadius: 8, border: '1px solid #e8eaed', fontSize: 13, width: 280, background: '#fff', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }} />
          {search && <button onClick={() => setSearch('')} style={{ marginLeft: 8, ...S.btn('ghost'), padding: '6px 10px' }}>✕ Clear</button>}
        </div>
        ...
        {page === 'dashboard' && (
          <div>
            <div style={{ marginBottom: 24 }}>
              ...
              <p style={{ color: '#6b7280', margin: '4px 0 0', fontSize: 13 }}>Dargei Beis · Week of Jun 2, 2025 · {total} students</p>
            </div>
            {/* Stats */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 10, marginBottom: 24 }}>
              {[['Total', total, '#1a1f36'],['Present', present, '#2563eb'],['Absent', absent, '#dc2626'],['Late', late, '#d97706'],['In Therapy', inTherapy, '#7c3aed'],['In Hallway', inHallway, '#0891b2'],['Not Arrived', notArrived, '#6b7280']].map(([label, val, color]) => (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  ...
                  {searchedStudents.map((s, i) => {
                    ...
                    return (
                      <div key={s.id} onClick={() => setSelectedStudent(s)} style={{ background: '#fafafa', border: '1px solid #e8eaed', borderRadius: 8, padding: '10px 12px', cursor: 'pointer' }}>
                        ...
                        {withStaffObj && <div style={{ fontSize: 10, color: '#0891b2', fontWeight: 600, marginBottom: 4 }}>👤 {withStaffObj.name}</div>}
                        <div style={{ fontSize: 10, color: imp.color, fontWeight: 600, marginBottom: 4 }}>{imp.label}</div>
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                          <span style={S.badge('#92400e', '#fef3c7')}>{s.points} pts</span>
                          {s.reminders >= 4 && <span style={S.badge('#dc2626', '#fee2e2')}>⚠️ {s.reminders}</span>}
              ...
              <div style={S.card}>
                <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 12 }}>Top Alerts</div>
              ...
              <div style={S.card}>
                <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 12 }}>Parent Calls Needed</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 260, overflow: 'auto' }}>
                  {students.map((s, i) => {
                    const lastCall = s.parentCalls.length > 0 ? s.parentCalls[s.parentCalls.length - 1] : null
                  ...
                  <div key={s.id} onClick={() => setSelectedStudent(s)} style={{ ...S.card, display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', padding: '14px 18px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, marginTop: 4, flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 600, color: imp.color }}>{imp.label}</span>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
                  ...
                  <div>
                    ...
                    <div style={{ color: '#d97706', fontWeight: 700, fontSize: 13 }}>{students.find(s => s.id === behaviorStudent.id)?.points} pts · {students.find(s => s.id === behaviorStudent.id)?.reminders} reminders</div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
                  <button onClick={() => setBehaviorTab('positive')} style={S.btn(behaviorTab === 'positive' ? 'success' : 'ghost')}>✅ Positive</button>
                  <button onClick={() => setBehaviorTab('negative')} style={S.btn(behaviorTab === 'negative' ? 'danger' : 'ghost')}>❌ Reminders</button>
                </div>
                ...
                {STORE_ITEMS.map(item => {
                  ...
                  return (
                    <div key={item.id} style={{ ...S.card, textAlign: 'center', opacity: canAfford ? 1 : 0.5 }}>
                      ...
                      <button onClick={() => buyItem(storeStudent, item.cost, item.name)} style={{ ...S.btn(canAfford ? 'success' : 'ghost'), width: '100%', cursor: canAfford ? 'pointer' : 'not-allowed', fontSize: 12 }}>{canAfford ? 'Redeem' : 'Not enough'}</button>
                    </div>
                  )
                })}
              </div>

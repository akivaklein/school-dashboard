...
export default function PrintDaveningChecklist({ students, classes, onClose, S, primaryClassIdsByStudent, additionalClassIdsByStudent, instructionalGroups, instructionalGroupMemberships }: { students: Student[]; classes: SchoolClass[]; onClose: () => void; S: any; primaryClassIdsByStudent: Record<string | number, string>; additionalClassIdsByStudent: Record<string | number, string[]>; instructionalGroups: Array<{ id: string; status?: string }>; instructionalGroupMemberships: Array<{ group_id: string; student_id: number }> }) {
  const [scope, setScope] = useState<'school' | 'class' | 'teacher'>('class')
  ...
  const extraStudents = activeStudents.filter(student => extraStudentIds.has(String(student.id)) && !matchingStudents.some(matched => String(matched.id) === String(student.id)))
  const printableStudents = [...selectedStudents, ...extraStudents]
  const extraStudentSearchResults = extraStudentSearch.trim()
    ? activeStudents.filter(student => String(student.name || '').toLowerCase().includes(extraStudentSearch.trim().toLowerCase()) && !matchingStudents.some(matched => String(matched.id) === String(student.id)) && !extraStudentIds.has(String(student.id))).slice(0, 8)
    : []
  ...
  function printChecklist() {
    ...
    const studentRows = printableStudents.map((student, index) => `<tr>${showNumbering ? `<td class="number">${index + 1}</td>` : ''}<td class="name">${escapeHtml(student.name)}</td>${cleanSections.map(() => '<td class="mark-space"></td>').join('')}</tr>`).join('')
    const blankRows = Array.from({ length: blankRowCount }).map((_, index) => `<tr>${showNumbering ? `<td class="number">${printableStudents.length + index + 1}</td>` : ''}<td class="name mark-space"></td>${cleanSections.map(() => '<td class="mark-space"></td>').join('')}</tr>`).join('')
    const rows = studentRows + blankRows
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 700, background: 'rgba(15,23,42,0.45)', display: 'grid', placeItems: 'center', padding: 18 }}>
      <section role="dialog" aria-modal="true" aria-label="Print davening checklist" style={{ width: 'min(94vw, 820px)', maxHeight: '90vh', overflow: 'auto', background: '#fff', borderRadius: 10, boxShadow: '0 24px 70px rgba(15,23,42,0.3)' }}>
        ...
        <div style={{ padding: 22, display: 'grid', gap: 18 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>{([['class', 'Specific Class'], ['teacher', 'Specific Teacher']] as const).map(([value, label]) => <button key={value} onClick={() => setScope(value)} style={{ ...S.btn(scope === value ? 'primary' : 'ghost'), padding: '9px 8px', fontSize: 12 }}>{label}</button>)}</div>
          ...
          <div><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}><div style={{ fontSize: 13, fontWeight: 800, color: '#334155' }}>Davening sections</div><button onClick={() => setSections(previous => [...previous, `Section ${previous.length + 1}`])} style={{ ...S.btn('ghost'), padding: '5px 9px', fontSize: 11 }}>Add Section</button></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 8 }}>{sections.map((section, index) => <div key={index} style={{ display: 'flex', gap: 4 }}><input aria-label={`Davening section ${index + 1}`} value={section} onChange={event => setSections(previous => previous.map((entry, sectionIndex) => sectionIndex === index ? event.target.value : entry))} style={{ minWidth: 0, flex: 1, padding: '8px 9px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 12 }} /><button title="Move section left" onClick={() => setSections(previous => movePrintColumn(previous, index, index - 1))} disabled={index === 0} aria-label={`Move ${section || 'section'} left`} style={{ ...S.btn('ghost'), padding: '5px 8px', opacity: index === 0 ? 0.4 : 1 }}>&lt;</button><button title="Move section right" onClick={() => setSections(previous => movePrintColumn(previous, index, index + 1))} disabled={index === sections.length - 1} aria-label={`Move ${section || 'section'} right`} style={{ ...S.btn('ghost'), padding: '5px 8px', opacity: index === sections.length - 1 ? 0.4 : 1 }}>&gt;</button><button onClick={() => setSections(previous => previous.filter((_, sectionIndex) => sectionIndex !== index))} aria-label={`Remove ${section || 'section'}`} style={{ ...S.btn('ghost'), padding: '5px 8px', color: '#9f1239' }}>x</button></div>)}</div></div>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 800, color: '#334155', marginBottom: 8 }}>Temporarily add a student from another class</div>
            <input value={extraStudentSearch} onChange={event => setExtraStudentSearch(event.target.value)} placeholder="Search student name..." style={{ width: '100%', boxSizing: 'border-box', padding: '9px 10px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 13 }} />
            {extraStudentSearchResults.length > 0 && <div style={{ border: '1px solid #e2e8f0', borderRadius: 7, marginTop: 6 }}>{extraStudentSearchResults.map(student => <div key={student.id} onClick={() => { setExtraStudentIds(previous => new Set(previous).add(String(student.id))); setExtraStudentSearch('') }} style={{ padding: '8px 10px', borderBottom: '1px solid #f1f5f9', cursor: 'pointer', fontSize: 13 }}>{student.name}</div>)}</div>}
            {extraStudents.length > 0 && <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 8 }}>{extraStudents.map(student => <span key={student.id} style={{ padding: '4px 8px', borderRadius: 999, background: '#eff6ff', color: '#1d4ed8', fontSize: 12, fontWeight: 700 }}>{student.name} <button onClick={() => setExtraStudentIds(previous => { const next = new Set(previous); next.delete(String(student.id)); return next })} aria-label={`Remove ${student.name}`} style={{ border: 'none', background: 'transparent', color: '#1d4ed8', cursor: 'pointer' }}>x</button></span>)}</div>}
          </div>
          <label style={{ display: 'grid', gap: 6, maxWidth: 220, fontSize: 12, fontWeight: 700, color: '#334155' }}>Blank handwritten rows<input type="number" min={0} max={30} value={blankRowCount} onChange={event => setBlankRowCount(Math.max(0, Math.min(30, Number(event.target.value) || 0)))} style={{ padding: '9px 10px', border: '1px solid #cbd5e1', borderRadius: 6 }} /></label>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}><button onClick={onClose} style={S.btn('ghost')}>Cancel</button><button disabled={printableStudents.length === 0 && blankRowCount === 0} onClick={printChecklist} style={{ ...S.btn('primary'), opacity: printableStudents.length === 0 && blankRowCount === 0 ? 0.55 : 1 }}>Print Checklist</button></div>
        </div>
      </section>
    </div>
  )

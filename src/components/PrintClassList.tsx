...
function getStudentClassId(student: Student) {
  return String(student.classId || student.class_id || '')
}

export default function PrintClassList({ students, classes, onClose, S }: { students: Student[]; classes: SchoolClass[]; onClose: () => void; S: any }) {
  ...
  const matchingStudents = useMemo(() => {
    if (scope === 'class') return activeStudents.filter(student => getStudentClassId(student) === classId)
    if (scope === 'teacher') {
      const teacherClassIds = new Set(classes.filter(entry => entry.teacher === teacherName).map(entry => String(entry.id)))
      return activeStudents.filter(student =>
        teacherClassIds.has(getStudentClassId(student))
        || (student.services || []).some(service => String(service.role || '').toLowerCase() === 'teacher' && (service.staffName || service.name) === teacherName),
      )
    }

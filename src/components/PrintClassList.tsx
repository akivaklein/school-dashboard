...
export default function PrintClassList({ students, classes, onClose, S, primaryClassIdsByStudent, additionalClassIdsByStudent, instructionalGroups, instructionalGroupMemberships }: { students: Student[]; classes: SchoolClass[]; onClose: () => void; S: { btn: (variant?: string) => CSSProperties }; primaryClassIdsByStudent: Record<string | number, string>; additionalClassIdsByStudent: Record<string | number, string[]>; instructionalGroups: Array<{ id: string; status?: string }>; instructionalGroupMemberships: Array<{ group_id: string; student_id: number }> }) {
  ...
  // Extra students added for this print run only — never changes their real class assignment.
  const extraStudents = useMemo(
    () => activeStudents.filter(student => extraStudentIds.has(String(student.id)) && !matchingStudents.some(matched => String(matched.id) === String(student.id))),
    [activeStudents, extraStudentIds, matchingStudents],
  )
  const printableStudents = [...selectedStudents, ...extraStudents]
  const extraStudentSearchResults = extraStudentSearch.trim().length === 0
    ? []
    : activeStudents.filter(student => (
        String(student.name || '').toLowerCase().includes(extraStudentSearch.trim().toLowerCase())
        && !matchingStudents.some(matched => String(matched.id) === String(student.id))
        && !extraStudentIds.has(String(student.id))
      )).slice(0, 8)
  ...
  function printList() {
    ...
    const studentRows = printableStudents.map((student, index) => `
    ...
    `).join('')
    const blankRows = Array.from({ length: Math.max(0, blankRowCount) }).map((_, index) => `
      <tr>${showNumbering ? `<td class="number">${printableStudents.length + index + 1}</td>` : ''}<td class="name blank"></td>${cleanColumns.map(() => '<td class="blank"></td>').join('')}</tr>
    `).join('')
    ...
    </style></head><body><h1>${escapeHtml(title || 'Class List')}</h1><div class="meta">${escapeHtml(selectionLabel)} | ${escapeHtml(today)} | ${printableStudents.length} students${blankRowCount > 0 ? ` + ${blankRowCount} blank` : ''}</div><table class="${showNumbering ? '' : 'without-numbering'}"><thead><tr>${showNumbering ? '<th class="number">#</th>' : ''}<th>Student</th>${headerCells}</tr></thead><tbody>${rows}</tbody></table></body></html>`)
    win.document.close()
    win.focus()
    win.print()
  }
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 700, background: 'rgba(15,23,42,0.45)', display: 'grid', placeItems: 'center', padding: 18 }}>
      <section role="dialog" aria-modal="true" aria-label="Print class list" style={{ width: 'min(94vw, 820px)', maxHeight: '90vh', overflow: 'auto', background: '#fff', borderRadius: 10, boxShadow: '0 24px 70px rgba(15,23,42,0.3)' }}>
        ...
        <div style={{ padding: 22, display: 'grid', gap: 18 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 800, color: '#334155', marginBottom: 8 }}>Temporarily add a student from another class (this printout only)</div>
            ...
            {extraStudentSearchResults.length > 0 && (
              <div style={{ border: '1px solid #e2e8f0', borderRadius: 7, marginTop: 6, maxHeight: 160, overflow: 'auto' }}>
                {extraStudentSearchResults.map(student => (
                  <div key={student.id} onClick={() => { setExtraStudentIds(previous => new Set(previous).add(String(student.id))); setExtraStudentSearch('') }} style={{ padding: '8px 10px', borderBottom: '1px solid #f1f5f9', fontSize: 13, cursor: 'pointer' }}>{student.name}</div>
                ))}
              </div>
            )}
            {extraStudents.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                {extraStudents.map(student => (
                  <span key={student.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 8px', borderRadius: 999, background: '#eff6ff', color: '#1d4ed8', fontSize: 12, fontWeight: 700 }}>
                    {student.name}
                    <button onClick={() => setExtraStudentIds(previous => { const next = new Set(previous); next.delete(String(student.id)); return next })} aria-label={`Remove ${student.name} from this printout`} style={{ border: 'none', background: 'transparent', color: '#1d4ed8', cursor: 'pointer', fontWeight: 800 }}>×</button>
                  </span>
                ))}
              </div>
            )}
          </div>
          <label style={{ display: 'grid', gap: 6, fontSize: 12, fontWeight: 700, color: '#334155', maxWidth: 220 }}>Blank handwritten rows
            <input type="number" min={0} max={30} value={blankRowCount} onChange={event => setBlankRowCount(Math.max(0, Math.min(30, Number(event.target.value) || 0)))} style={{ padding: '9px 10px', border: '1px solid #cbd5e1', borderRadius: 6, fontSize: 14 }} />
          </label>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}><button onClick={onClose} style={S.btn('ghost')}>Cancel</button><button disabled={printableStudents.length === 0 && blankRowCount === 0} onClick={printList} style={{ ...S.btn('primary'), opacity: (printableStudents.length === 0 && blankRowCount === 0) ? 0.55 : 1 }}>Print List</button></div>
        </div>
      </section>
    </div>
  )

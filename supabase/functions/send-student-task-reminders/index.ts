...
import { buildEmailHtml, getNotificationTime, isReadyToSend, providerFailureMessage, type ReminderTask } from './logic.ts'
...
Deno.serve(async request => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !resendApiKey || !fromEmail || !recipientEmail) return response({ error: 'Email reminder server configuration is incomplete.' }, 500)
  ...
  const { data, error } = await admin
    ...
    .in('notification_preference', ['email', 'email_text'])
    .limit(1000)
  ...
  for (const row of (data || []) as TaskRow[]) {
    ...
    const scheduledFor = getNotificationTime(row).toISOString()
    const { data: claimed, error: claimError } = await admin.rpc('claim_student_task_email_delivery', {
      p_task_id: row.id,
    ...
    try {
      ...
      await admin.from('student_task_email_deliveries').update({ status: 'sent', provider_message_id: resendBody.id || null, sent_at: new Date().toISOString(), last_error: null }).eq('task_id', row.id).eq('notification_cycle', row.notification_cycle)
      summary.sent += 1
    } catch (sendError) {
      await admin.from('student_task_email_deliveries').update({ status: 'failed', last_error: sendError instanceof Error ? sendError.message : 'Email provider failure' }).eq('task_id', row.id).eq('notification_cycle', row.notification_cycle)
      summary.failed += 1
    }
  }
